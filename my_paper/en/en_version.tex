...
\begin{abstract}
Computed tomography (CT) technology is essential for clinical diagnosis.However, X-ray ionizing radiation is harmful and may increase the risk of cancers.In order to reduce the impact of radiation on human body,researchers obtained sparsely sampled sinogram by reducing the number of detectors,but the sinogram of sparse-view will bring problems such as blurred images and streak artifacts, which cause a negative impact on the diagnosis.In this paper, with purpose to reconstruct high quality CT images from sparsely sampled sinograms, we propose a Transformer-based module (called: DDPTransformer Block) to capture the long-term dependencies within the images and train and validate it on the "Low Dose CT Image and Projection Data (LDCT-and-Projection-data)" dataset.More specifically, we replace the traditional convolution operation with the parallel Transformers,and augment the edge information of patch blocks by different splits.In addition, considering that they are two-dimensional images, we propose the Layer-Conv-Layer (LCL) module to replace the MLP in Transformer for feature extraction.Finally, with DDPTransformer Block as the backbone, we propose a deep learning model with four stages including Interpolation, Sinogram Domain Subnet, Filter BackProjection (FBP) and Image Domain SubNet.Experimental results proved that DDPTransformer achieves superior or comparable performance compared to state-of-the-art algorithms for different sparsely sampled sinograms.And finally through other data sets to further verify the robustness of the model.The code and model are publicly available at：xxxxx
\end{abstract}
...
但CT的电离辐射会对人体造成危害，这严重限制了它的实际应用。在临床诊断中，为了去降低辐射剂量和减少扫描时间而采用sparse view CT。然而，投影视图的缺陷给重建的图像带来ill-posed inverse problems\cite{1989Incomplete}。许多重建算法被提出用来解决这些问题，他们一般被分为3类：(a) sinogram domain pre-processing,(b) iterative algorithm, and (c)image domain post-processing.
...
\bibitem{2018Deep}
Mihalj Bakator and Dragica Radosav, {\it{Deep learning and medical diagnosis: A review of literature.}}, Multimodal Technologies and Interaction, 2(3), 2018.
